...
const makeFakeServiceRepo = (saved: Service[] = []): ServiceRepository => ({
  findById: async () => null,
...
describe('CreateBarbershopUseCase', () => {
  ...
  test('cria os serviços do catálogo fixo escolhidos na criação', async () => {
    const saved: Service[] = []
    const useCase = new CreateBarbershopUseCase(makeFakeRepo(), makeFakeUserRepo(buildOwner()), makeFakeServiceRepo(saved))
    await useCase.execute({ name: 'Barbearia com Serviços', ownerUserId: OWNER_ID, serviceNames: ['Corte', 'Barba'] })

    assert.equal(saved.length, 2)
    assert.deepEqual(saved.map((s) => s.name).sort(), ['Barba', 'Corte'])
  })

  test('ignora nomes de serviço que não fazem parte do catálogo fixo', async () => {
    const saved: Service[] = []
    const useCase = new CreateBarbershopUseCase(makeFakeRepo(), makeFakeUserRepo(buildOwner()), makeFakeServiceRepo(saved))
    await useCase.execute({ name: 'Barbearia Inválida', ownerUserId: OWNER_ID, serviceNames: ['Corte', 'Manicure'] })

    assert.equal(saved.length, 1)
    assert.equal(saved[0]?.name, 'Corte')
  })
})
